...
public class Instructions extends World
{
    ...
    private Label toContinue = new Label("Press 'space' to continue",30);
    ...
    public void moveLabelUp(){
        if(toContinue.getY() > 340 && labelMoveDirection == -1){
            toContinue.setLocation(toContinue.getX(),toContinue.getY()-1);
        }else if(toContinue.getY() == 340){
            labelMoveDirection = 1;
        }
    }
    
    public void moveLabelDown(){
        if(toContinue.getY() < 350 && labelMoveDirection == 1){
            toContinue.setLocation(toContinue.getX(),toContinue.getY()+1);
        }else if(toContinue.getY() == 350){
            labelMoveDirection = -1;
        }
    }
}
